function merge_last_case2(node_lca, leafpath1, leafpath1idx, p_remain_1, t1_pzgw, node_t1,...
...
if strcmp(node_t2.parent.name, node_lca.name) == 1
    node_lca.alpha1(pindex_2) = [];
    node_lca.children(pindex_2) = [];
    node_lca.twmatparent(pindex_2, :) = [];
end
node_lca.alpha1 = bsxrdivide(node_lca.alpha1, sum(node_lca.alpha1));
pzgw = [];
if isempty(t1_pzgw) == 0
    pzgw = [pzgw; t1_pzgw];
end
